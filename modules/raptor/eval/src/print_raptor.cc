...
void print(schedule const& sched, raptor_schedule const& rp_sched,
           journey const& j) {
  std::cout << "Journey with TR: " << j.trips_.size()
            << ";\tMOC: " << j.occupancy_max_ << ";\tDuration: " << j.duration_
            << ";\n=========================================================\n";
  ...
  for (int idx = 0, size = j.trips_.size(); idx < size; ++idx) {
    ...
    if (prev_arr_eva != j_from_stop.eva_no_) {
      ...
      std::cout << "     From:\ts_id: " << std::setw(6)
                << +rp_sched.eva_to_raptor_id_.at(prev_arr_eva)
                << ";\teva: " << prev_arr_eva
                << ";\tDep: " << std::setw(7)
                ...
                << "     Using: Footpath\n"
                << "     To:\ts_id: " << std::setw(6)
                << +rp_sched.eva_to_raptor_id_.at(j_from_stop.eva_no_)
    ...
    std::cout << std::setw(3) << idx << ": "
              ...
              << " (" << j_from_stop.departure_.timestamp_ << ");\n"
              << "     Using: " << s_trip_dbg
              << rp_sched.dbg_.str(s_trip_dbg) << ";\n"
              << "     To:\ts_id: " << std::setw(6)
              << +rp_sched.eva_to_raptor_id_.at(j_to_stop.eva_no_)
              << ";\teva: " << j_to_stop.eva_no_
              << ";\tArr: " << std::setw(7)
              << +unix_to_motistime(sched.schedule_begin_,
...
int main(int argc, const char** argv) {
  ...
  auto const rp_sched = get_raptor_schedule(sched, false, "");
