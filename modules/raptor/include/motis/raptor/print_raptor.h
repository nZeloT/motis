...
inline std::string get_string(station_id const s_id, raptor_schedule const& sched) {
...
inline void print_route(route_id r_id, raptor_timetable const& tt) {
  ...
  for (trip_count trip_offset = 0; 
                  trip_offset < route.trip_count_; 
                ++trip_offset) {
    std::cout << trip_offset << " \t[ ";
    for (station_id stop_offset = 0; stop_offset < stop_count; ++stop_offset) {
      auto st_idx = index_into_stop_times + (trip_offset * stop_count) + stop_offset;
      std::cout << stop_offset << ": "
...
inline std::vector<route_id> get_routes_containing(std::vector<station_id>
                                                   const& stations,
                                                   raptor_timetable const& tt){ 
  std::vector<route_id> routes;
  ...
  for (route_id r_id = 0; r_id < tt.route_count(); ++r_id) {
    ...
    auto const route_begin = std::begin(tt.route_stops_) 
                           + route.index_to_route_stops_;
    ...
    for (auto const s_id : stations) {
      ...
      if (found == route_end) { contains_all = false; break; }
    }

    if (contains_all) { routes.emplace_back(r_id); }
  }
